...
class ExtendedSpellCheckSuggestionsToolbar extends StatelessWidget {
  /// Constructs a [ExtendedSpellCheckSuggestionsToolbar].
  ///
  /// [buttonItems] must not contain more than four items, generally three
  /// suggestions and one delete button.
  const ExtendedSpellCheckSuggestionsToolbar({
    super.key,
    required this.anchor,
    required this.buttonItems,
  }) : assert(buttonItems.length <= _kMaxSuggestions + 1);

  /// Constructs a [ExtendedSpellCheckSuggestionsToolbar] with the default children for
  /// an [EditableText].
  ///
  /// See also:
  ///  * [CupertinoSpellCheckSuggestionsToolbar.editableText], which is similar
  ///    but builds an iOS-style toolbar.
  ExtendedSpellCheckSuggestionsToolbar.editableText({
    super.key,
    // zmtzawqlp
  ...
  // zmtzawqlp
  static void _replaceText(ExtendedEditableTextState editableTextState,
      String text, TextRange replacementRange) {
